...
export function toObject (str:string) : any {
  // max number of nodes...
  const parser = new XMLParser([str])
  let max_cnt = 1000000
  ...
  while(!parser.eof) {
    parser.parse({
      beginNode( name:string, buffer_index?:number ) {
        parents.push( activeNode )
        const newNode = createNode( name )
        activeNode.children.push( newNode )
        activeNode = newNode
      },
      setAttribute( name:string, value:bufferType, buffer_index?:number) {
        activeNode.attributes.push({name, value})
      },
      addTextNode( value:string, buffer_index?:number ) {
        activeNode.children.push(createTextNode(value))
      },
      closeNode( name:string, buffer_index?:number ) {
        activeNode = parents.pop()
      }        
    })
    if(max_cnt-- < 0 ) break
  }  
